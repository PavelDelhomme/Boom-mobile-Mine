...
class OfflineCacheService with ChangeNotifier {
  ...
  bool _isOnline = true;
  Map<String, Uint8List> _memoryTileCache = {}; // Cache mémoire pour performances

  // Limites de cache
  static const int MAX_MEMORY_CACHE_SIZE = 100; // 100 tuiles en mémoire
  static const int MAX_DISK_CACHE_MB = 500; // 500 MB max sur disque
  static const Duration CACHE_VALIDITY_DURATION = Duration(days: 30);
  ...
  Future<Uint8List?> _getTileFromDisk(String tileId) async {
    try {
      ...
      if (result.isNotEmpty) {
        ...
        if (now - cachedAt < CACHE_VALIDITY_DURATION.inMilliseconds) {
  ...
  void _addToMemoryCache(String tileId, Uint8List data) {
    if (_memoryTileCache.length >= MAX_MEMORY_CACHE_SIZE) {
  ...
  Future<void> _cleanupCacheIfNeeded() async {
    try {
      ...
      if (totalSizeMB > MAX_DISK_CACHE_MB) {
